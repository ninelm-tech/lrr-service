...
describe('AuthService', () => {
  ...
  describe('requestPasswordReset', () => {
    it('rejects a password shorter than 8 characters regardless of flag state', async () => {
      await expect(service.requestPasswordReset('ada@example.com', 'short')).rejects.toThrow('at least 8 characters');
    ...
    describe('with the flag off (default) — resets immediately, no verification', () => {
      it('resolves by email, updates the password directly, and reports otpRequired:false', async () => {
        prisma.user.findUnique.mockResolvedValueOnce({ id: 'u-1', email: 'ada@example.com', passwordHash: 'old-hash' });

        const result = await service.requestPasswordReset('ada@example.com', 'newpassword1');
        ...
        expect(result).toEqual({ message: 'Password updated. You can now log in.', otpRequired: false });
      });
      ...
      it('falls back to phone lookup when the identifier does not match an email', async () => {
        prisma.user.findUnique
          ...
          .mockResolvedValueOnce({ id: 'u-1', phoneNumber: '+2348012345678', passwordHash: 'old-hash' });

        await service.requestPasswordReset('+2348012345678', 'newpassword1');

        expect(prisma.user.findUnique).toHaveBeenNthCalledWith(2, { where: { phoneNumber: '+2348012345678' } });
        expect(prisma.user.update).toHaveBeenCalledWith({
          where: { id: 'u-1' },
          data: { passwordHash: expect.any(String) },
        });
      ...
      it('no-ops (but still reports success) when no account matches', async () => {
        ...
        const result = await service.requestPasswordReset('nobody@example.com', 'newpassword1');
      ...
      it('no-ops when the account has no portal password', async () => {
        prisma.user.findUnique.mockResolvedValueOnce({ id: 'u-1', passwordHash: null });
    ...
    describe('with the flag on — sends a code, never resets directly', () => {
      ...
      it('resolves by email first, sends the code, ignores newPassword, and reports otpRequired:true', async () => {
        prisma.user.findUnique.mockResolvedValueOnce({ id: 'u-1', email: 'ada@example.com', phoneNumber: '+2348012345678', passwordHash: 'hashed' });

        const result = await service.requestPasswordReset('ada@example.com', 'newpassword1');

        expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { email: 'ada@example.com' } });
        expect(otpService.sendPasswordResetCode).toHaveBeenCalledWith('+2348012345678');
        expect(prisma.user.update).not.toHaveBeenCalled();
        expect(result).toEqual({ message: "If an account exists, we've sent a reset code to its registered phone number.", otpRequired: true });
      });

      it('falls back to phone lookup when the identifier does not match an email', async () => {
        prisma.user.findUnique
          .mockResolvedValueOnce(null) // email lookup misses
          .mockResolvedValueOnce({ id: 'u-1', phoneNumber: '+2348012345678', passwordHash: 'hashed' });
      ...
      it('returns the same generic message and sends nothing when the account has no portal password', async () => {
        prisma.user.findUnique.mockResolvedValueOnce({ id: 'u-1', phoneNumber: '+2348012345678', passwordHash: null });
  ...
  describe('resetPasswordWithCode', () => {
    it('rejects a password shorter than 8 characters', async () => {
      await expect(service.resetPasswordWithCode('+2348012345678', '123456', 'short')).rejects.toThrow('at least 8 characters');
    ...
    it('fails to find a code when the flag was off at request time (nothing was ever sent)', async () => {
      otpService.verifyCode.mockRejectedValue(new Error('Code expired or not found — request a new one.'));

      await expect(service.resetPasswordWithCode('+2348012345678', '123456', 'newpassword1')).rejects.toThrow('Code expired or not found');
    });
    ...
    describe('with the flag on', () => {
      ...
      it('verifies the code, consumes the token, and updates the password', async () => {
        ...
        const result = await service.resetPasswordWithCode('+2348012345678', '123456', 'newpassword1');
      ...
      it('rejects when the token was already consumed between verify and the transaction', async () => {
        ...
        const tx = {
          phoneVerification: {
            findUnique: jest.fn().mockResolvedValue({ id: 'pv-1', consumedAt: new Date(), tokenExpiresAt: new Date(Date.now() + 60_000) }),
            update: jest.fn(),
          },
        ...
        await expect(service.resetPasswordWithCode('+2348012345678', '123456', 'newpassword1')).rejects.toThrow('expired');
